 ...
 * fixed Toolbar, BubbleToolbar, etc.) already receives `editor` as a
 * required prop directly. So this version just returns the provided
 * editor with no context fallback — same shape (`{ editor }`) the ported
 * components expect, simpler because this app doesn't need the other
 * half of what the real hook does.
 */
