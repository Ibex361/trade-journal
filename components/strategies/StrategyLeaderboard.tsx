...
import { StrategyLeaderboardRow, STRATEGY_MIN_SAMPLE_SIZE } from "@/lib/metrics";
...
export default function StrategyLeaderboard({
  ...
  if (rows.length === 0) {
    return (
      <Card title="Strategy leaderboard" description="Win rate, expectancy, and risk metrics per strategy">
        <div className="h-40 flex items-center justify-center">
          <p className="text-ink-muted text-sm">No trades yet.</p>
        </div>
      </Card>
    );
  }
    ...
    >
      ...
      {lowSampleCount > 0 && !reliableOnly && (
        <p className="text-xs text-ink-muted mt-3">
          {lowSampleCount} strateg{lowSampleCount === 1 ? "y has" : "ies have"} fewer than {STRATEGY_MIN_SAMPLE_SIZE} trades —
