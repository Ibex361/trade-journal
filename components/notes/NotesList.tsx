  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {notes.map((note) => {
        ...
        return (
          <button key={note.id} type="button" onClick={() => onSelectNote(note)} className="text-left">
            <Card
              padding="tight"
              className="space-y-2 h-full hover:border-glow/40 transition-colors duration-fast"
            >
